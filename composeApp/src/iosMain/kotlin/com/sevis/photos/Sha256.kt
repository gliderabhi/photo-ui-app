...
// Minimal, dependency-free SHA-256 (FIPS 180-4) — used only to compute the OAuth
// PKCE code_challenge (S256) in GoogleAuth.kt. Avoids pulling in CommonCrypto
// (needs a custom cinterop module map) or a third-party crypto library for one
// hash call.
...
fun sha256(message: ByteArray): ByteArray {
    ...
    var totalLen = message.size + 1 + 8
    val remainder = totalLen % 64
    if (remainder != 0) totalLen += 64 - remainder
    ...
    for (i in 0 until 8) {
        result[i * 4] = (hs[i] ushr 24).toByte()
        result[i * 4 + 1] = (hs[i] ushr 16).toByte()
        result[i * 4 + 2] = (hs[i] ushr 8).toByte()
